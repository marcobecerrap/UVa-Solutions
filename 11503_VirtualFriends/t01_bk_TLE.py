...
for CC in range( TotalCases ):
    ...
    for II in range( TotalInteractions ):
        ...
        if( NewFriends[ 0 ] in People ):
            ...
            Network[ NewFriends[ 0 ] ] = [ NewFriends[ 0 ] ]
        ...
        if( NewFriends[ 1 ] in People ):
            ...
            Network[ NewFriends[ 1 ] ] = [ NewFriends[ 1 ] ]

        if( Friend0_Key == Friend1_Key ): # They belong to the same network
            print( str( len( Network[ Friend0_Key ] ) ) )
            continue
        else:
            ...
            NewKey = Friend0_Key
            OldKey = Friend1_Key
            if( len( Network[ Friend1_Key ] ) > len( Network[ Friend0_Key ] ) ): 
                NewKey = Friend1_Key
                OldKey = Friend0_Key

            # Merge groups
            Network[ NewKey ] = Network[ OldKey ] + Network[ NewKey ] # These are disjoint sets (lists), so they don't have repeated members
                
            # Update keys of old key
            for MM in Network[ OldKey ]:
                PeopleKey[ MM ] = NewKey
            del Network[ OldKey ]

            print( str( len( Network[ NewKey ] ) ) )
